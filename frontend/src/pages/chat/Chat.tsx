...
const Chat = () => {
    ...
    const getUserInfoList = async () => {
        if (appStateContext?.state?.appSettings?.enforce_auth) {
                ...
                <Stack horizontal className={styles.chatRoot}>
                    <div className={styles.chatContainer}>
                        {!messages || messages.length < 1 ? (
                            <Stack className={styles.chatEmptyState}>
                                <img
                                    ...
                                    aria-hidden="true"
                                />
                                <h1 className={styles.chatEmptyStateTitle}>{appStateContext?.state?.appSettings?.welcome_title}</h1>
                                <h2 className={styles.chatEmptyStateSubtitle}>{appStateContext?.state?.appSettings?.welcome_subtitle}</h2>
                            </Stack>
                        ) : (
                                        ...
                                        {answer.role === "user" ? (
                                            ...
                                            answer.role === "assistant" ? <div className={styles.chatMessageGpt}>
                                                <Answer
                                                    ...
                                                    disclaimer={"Generated by" + appStateContext?.state?.appSettings?.bot_name}
                                                    showCitations={appStateContext?.state?.appSettings?.show_references ?? true}
                        ...
                        <Stack horizontal className={styles.chatInput}>
                            ...
                            <QuestionInput
                                clearOnSend
                                placeholder={appStateContext?.state?.appSettings?.chatbox_placeholder}
                                disabled={isLoading}
                                onSend={(question, id) => {
